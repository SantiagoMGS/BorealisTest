...
export interface IUserRepository extends IRepository<User> {
  ...
  assignUserToCompanies(userId: string, permissions: { companyId: string; roleId: string }[]): Promise<void>;
  ...
  updateRefreshToken(userId: string, refreshToken: string, expiry: Date): Promise<void>;
  findUserByRefreshToken(refreshToken: string): Promise<Omit<User, 'hashedPassword'> | null>;
